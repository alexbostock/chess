import Position, { allValidPositions } from './Position';
  ...
  switch (piece.type) {
    case 'pawn':
      return allValidPositions.filter(({ x, y }) => {
        ...
        const nextRow = piece.colour === 'white' ? currentRow + 1 : currentRow - 1;
        const twoRowsAhead = piece.colour === 'white' ? nextRow + 1 : nextRow - 1;
        return (y === nextRow || (currentRow === startingRow && y === twoRowsAhead)) && x === p.x;
      });
    case 'bishop':
      return allValidPositions.filter(({ x, y }) => Math.abs(x - p.x) === Math.abs(y - p.y) && x !== p.x);
    case 'knight':
      return allValidPositions.filter(({ x, y }) => {
    ...
    case 'rook':
      return allValidPositions.filter(({ x, y }) => (x === p.x || y === p.y) && !(x === p.x && y === p.y));
    case 'queen':
      return allValidPositions.filter(
        ({ x, y }) => (x === p.x || y === p.y || Math.abs(x - p.x) === Math.abs(y - p.y)) && !(x === p.x && y === p.y)
      );
    case 'king':
      return allValidPositions.filter(({ x, y }) => {
        const xDiff = Math.abs(x - p.x);
